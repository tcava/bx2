 ...
 ************
 *
 * My hash routines for hashing Nicks, and eventually Channel's
 * and WhowasList's
 *
 ...
 */
static inline void move_link_to_top(Nick *tmp, Nick *prev, HashEntry *location)
{
	if (prev) 
	{
		Nick *old_list;
		old_list = (Nick *) location->list;
 ...
 */
static inline void remove_link_from_list(Nick *tmp, Nick *prev, HashEntry *location)
{
	if (prev) 
	{
...
void BX_add_nicklist_to_channellist(Nick *nptr, Channel *cptr)
{
	 ...
	 */
	nptr->next = (Nick *) cptr->NickListTable[hvalue].list;
...
Nick *BX_find_nicklist_in_channellist(char *nick, Channel *cptr, int remove)
{
	HashEntry *location;
	register Nick *tmp, *prev = NULL;
	 ...
	 * "We found the bucket, now search the chain"
	 */
	for (tmp = (Nick *) location->list; tmp; prev = tmp, tmp = tmp->next) 
	{
...
Nick *BX_next_nicklist(Channel *cptr, Nick *nptr)
{
	...
	else if (!nptr) 
	{
		...
		while ((Nick *) cptr->NickListTable[hvalue].list == NULL) 
		{
			hvalue++;
			if (hvalue >= NICKLIST_HASHSIZE)
				return NULL;
		}
		return (Nick *) cptr->NickListTable[hvalue].list;
	}
	else if (nptr->next) 
	{
...
void clear_nicklist_hashtable(Channel *cptr)
{
	if (cptr) 
	{
...
void show_nicklist_hashtable(Channel *cptr)
{
        int count, count2;
        Nick *ptr;
			...
			continue;
                    ...
                    count2 = 0; ptr; count2++, ptr = ptr->next) 
                {
                        say("HASH_DEBUG: %d:%d  %10s %s!%s", count, count2,
                            ptr->channel, ptr->nicklist->nick, ptr->nicklist->userhost);
                }
        }
}
...
extern WhowasWrapList whowas_splitin_list;
	if (args && *args)
		...
		c = get_echannel_by_refnum(0);
	if (c && from_server > -1)
		chan = find_channel(c, from_server);
	if (chan)
...
{
	unsigned long hvalue = hash_userhost_channel(wptr->nicklist->userhost, wptr->channel, WHOWASLIST_HASHSIZE);
...
WhowasList *BX_find_userhost_channel(char *host, char *channel, int remove, WhowasWrapList *wptr)
{
	...
	{
		if (!tmp->nicklist->userhost || !tmp->channel || !host || !channel)
			continue;
		if (!my_stricmp(host, tmp->nicklist->userhost) && !my_stricmp(channel, tmp->channel))
		{
			if (remove != REMOVE_FROM_LIST)
...
WhowasList *BX_next_userhost(WhowasWrapList *cptr, WhowasList *nptr)
{
	...
	else if (!nptr->next) 
	{
		int hvalue;
		/* hit end of chain, go to next bucket */
		hvalue = hash_userhost_channel(nptr->nicklist->userhost, nptr->channel, WHOWASLIST_HASHSIZE) + 1;
		if (hvalue >= WHOWASLIST_HASHSIZE) 
		{
			/* end of list */
			return NULL;
...
void show_whowas_hashtable(WhowasWrapList *cptr, char *list)
{
	...
	say("WhoWas %s Cache Stats: %lu hits  %lu  links  %lu unlinks", list, cptr->total_hits, cptr->total_links, cptr->total_unlinks);
		...
		if (cptr->NickListTable[count].links == 0)
			...
			put_it("%s", convert_output_format("%K[%W$[3]0%K] %Y$[10]1 %W$2%G!%c$3", "%d %s %s %s", count2, ptr->channel, ptr->nicklist->nick, ptr->nicklist->userhost));
        }
}
...
int show_wholeft_hashtable(WhowasWrapList *cptr, time_t ltime, int *total, int *hook, char *list)
{
                ...
                for (ptr = (WhowasList *) cptr->NickListTable[count].list, count2 = 1; ptr; count2++, ptr = ptr->next) 
		{
			if (ptr->server1/* && ptr->server2*/)
			{
				...
				if (do_hook(WHOLEFT_LIST, "%s %s %s %ld %s %s", ptr->nicklist->nick, ptr->nicklist->userhost, ptr->channel, ltime-ptr->time, ptr->server1?ptr->server1:"Unknown", ptr->server2?ptr->server2:"Unknown"))
					put_it("%s", convert_output_format(fget_string_var(FORMAT_WHOLEFT_USER_FSET), "%s %s %s %l %s", ptr->nicklist->nick, ptr->nicklist->userhost, ptr->channel, (long)ltime-ptr->time, ptr->server1?ptr->server1:empty_string));
			}
		}
        }
	if (*total)
...
time_t bx_now = time(NULL);
	if (!count)
	{
		for (x = 0; x < WHOWASLIST_HASHSIZE; x++)
		{
			...
			while (ptr)
			{
				if ((ptr->time + timet) <= bx_now)
				{
					if (!(ptr = find_userhost_channel(ptr->nicklist->userhost, ptr->channel, 1, list)))
						break;
					new_free(&(ptr->nicklist->ip));
					new_free(&(ptr->nicklist->nick));
					new_free(&(ptr->nicklist->userhost));
	...
	else
	{
		...
		{
			x = hash_userhost_channel(ptr->nicklist->userhost, ptr->channel, WHOWASLIST_HASHSIZE);
			if (!(ptr = find_userhost_channel(ptr->nicklist->userhost, ptr->channel, 1, list)))
				break;
			if (ptr->nicklist)
			{
...
{
NickSort *ns_a = (NickSort *)a, *ns_b = (NickSort* )b;
Nick *a1 = ns_a->nptr, *b1 = ns_b->nptr;
	return strcmp(a1->userhost, b1->userhost);
}
...
{
    int match = 0;
    char *nuh = malloc_strdup3(nick->nick, "!", nick->userhost);
