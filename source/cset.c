...
#include "window.h"  /* for get_echannel_by_refnum() */
#include "screen.h"  /* for curr_scr_win */
#include "names.h"   /* for find_channel() */
#include "log.h"
#include "ircaux.h"
#include "status.h"
#include "server.h"
...
static inline void cset_variable_case1(char *channel, int var_index, char *args)
{
	...
	{
		while (traverse_all_channels(&chan, current_window->server, 1))
		{
			tmp = var_index;
...
// XXX					set_cset_var_value(chan->csets, tmp, empty_string);
					set_cset_var_value(chan->csets, tmp, "");
				count++;
			}
		}
	}
/*	if (!count) */
	{
		...
		for (c = cset_queue; c; c = c->next)
		{
			...
			{
				for (tmp2 +=tmp; tmp < tmp2; tmp++)
// XXX					set_cset_var_value(c, tmp, empty_string);
					set_cset_var_value(c, tmp, "");
				count++;
			}
		}
		if (!count)
...
// XXX					set_cset_var_value(chan->csets, var_index, empty_string);
					set_cset_var_value(chan->csets, var_index, "");
				count++;
			}
		}
	}
/*	if (!count) */
	{
		...
		for (c = cset_queue; c; c = c->next)
		{
			if (!wild_match(channel, c->channel))
				continue;
			for (var_index = 0; var_index < NUMBER_OF_CSETS; var_index++)
// XXX				set_cset_var_value(c, var_index, empty_string);
				set_cset_var_value(c, var_index, "");
			count++;
		}
		if (!count)
...
char *set_cset(char *var, Channel *chan, char *value)
{
int var_index, cnt = 0;
...
char *get_cset(char *var, Channel *chan, char *value)
{
int var_index, cnt = 0;
	var_index = find_cset_variable(cset_array, var, &cnt);
	if (cnt == 1)
	{
		...
		*s = 0;
		switch (var->type)
		{
			...
			{
				char *t;
				t = malloc_strdup(get_cset_str_var(chan->csets, var_index));
				if (value)
					set_cset_str_var(chan->csets, var_index, value);
				return t;
			}
		}		
		return malloc_strdup(s && *s ? s : empty_string);
	}
...
// XXX			channel = get_echannel_by_refnum(0);
...
CSetList *create_csets_for_channel(char *channel)
{
	...
	tmp->log_level = malloc_strdup("ALL");	
	set_msglog_channel_level(cset_array, tmp); 
#if defined(WINNT) || defined(__EMX__)
	tmp->channel_log_file = malloc_sprintf(NULL, "~/bx-conf/%s.log", channel+1);
#else
	tmp->channel_log_file = malloc_sprintf(NULL, "~/.BitchX/%s.log", channel+1);
#endif
...
static void set_wset_var_value(Window *win, int var_index, const char *value)
{
	WSetArray *var;
...
// XXX			if (!(win = get_window_by_name(name)))
			if (!(win = get_window_by_desc(name)))
			{
...
void log_channel(CSetArray *var, CSetList *cs)
{
	...
	if ((chan = find_channel(cs->channel, from_server)))
...
void set_msglog_channel_level(CSetArray *var, CSetList *cs)
{
	...
	if ((chan = find_channel(cs->channel, from_server)))
	{
		int level = str_to_level(cs->log_level);
		if (level != -1)
			mask_set(&chan->log_level, level);
		else
			/* parse_lastlog_level set LOG_ALL in case of failure */
			mask_set(&chan->log_level, LEVEL_ALL);
...
//		set_cset_str_var(cs, CHANNEL_LOG_LEVEL_CSET, bits_to_lastlog_level(chan->log_level));
		set_cset_str_var(cs, CHANNEL_LOG_LEVEL_CSET, mask_to_str(&chan->log_level));
	}
}

void do_logchannel(unsigned long level, Channel *chan, char *format, ...)
{
	if (!chan || !get_cset_int_var(chan->csets, CHANNEL_LOG_CSET))
		return;
	if (mask_isset(&chan->log_level, level) && format)
//	if ((chan->log_level & level) && format)
	{
		...
		va_end(args);
// XXX		add_to_log(chan->msglog_fp, now, s, logfile_line_mangler);
		add_to_log(-1, chan->msglog_fp, -1, s, logfile_line_mangler, NULL);
	}
}

void set_channel_limit(Channel *channel, int currentlimit, int add, int numusers)
{
	if (time_diff(channel->limit_time, now) < 30)
		return;
	if (add && numusers)
	{
...
#endif
		NickList *nicks = &chan->nicks;
		set_channel_limit(chan, chan->limit, chan->csets->set_auto_limit, nicks->max);
	}
}
...
#endif
			set_channel_limit(chan, chan->limit, cs->set_auto_limit, nicks->max);
		} else
