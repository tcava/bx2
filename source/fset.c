...
BX_IrcVariable fset_array[] = 
{
	...
	{ "STATUS",		0,STR_TYPE_VAR,	0, NULL, build_status, 0, 0},
	{ "STATUS1",		0,STR_TYPE_VAR,	0, NULL, build_status, 0, 0},
	{ "STATUS2",		0,STR_TYPE_VAR,	0, NULL, build_status, 0, 0},
	{ "STATUS3",		0,STR_TYPE_VAR,	0, NULL, build_status, 0, 0},
...
void add_new_fset(char *name, char *args)
{
	if (args && *args)
	{
		BX_IrcVariable *tmp = NULL;
		int cnt, loc;
		tmp = (BX_IrcVariable *)find_array_item((Array *)&ext_fset_list, name, &cnt, &loc);
		if (!tmp || cnt >= 0)
		{
			tmp = new_malloc(sizeof(BX_IrcVariable));
	...
	{
		BX_IrcVariable *tmp;
		if ((tmp = (BX_IrcVariable *)remove_from_array((Array *)&ext_fset_list, name)))
		{
			new_free(&tmp->name);
			new_free(&tmp->string);
			new_free(&tmp);
...
BX_IrcVariable *find_ext_fset_var(char *name)
{
BX_IrcVariable *tmp = NULL;
int loc, cnt;
	tmp = (BX_IrcVariable *)find_array_item((Array *)&ext_fset_list, name, &cnt, &loc);
...
static int find_fset_variable(BX_IrcVariable *array, char *org_name, int *cnt)
{
	BX_IrcVariable *v, *first;
	int     len, var_index;
	char    *name = NULL;
...
Filename filename;
	...
	sprintf(thefile, "%s/%s.fmt", get_string_var(CTOOLZ_DIR_VAR), BX_version);
#else
	sprintf(thefile, "%s/%s.formats", get_string_var(CTOOLZ_DIR_VAR), BX_version);
#endif
	if (normalize_filename(thefile, filename) != 0)
	{
		bitchsay("Could not normalize %s for saving!", thefile);
		return 1;
	}
	...
	if (!outfile)
	{
		bitchsay("Cannot open file %s for saving!", filename);
		return 1;
	}
...
BX_IrcVariable *fget_var_address(char *var_name)
{
	BX_IrcVariable *var = NULL;
	int	cnt,
		msv_index;
	...
	if ((var = find_ext_fset_var(tmp_var)))
		return var;
	if ((BX_find_fixed_array_item (fset_array, sizeof(BX_IrcVariable), NUMBER_OF_FSET, tmp_var, &cnt, &msv_index) == NULL))
		return NULL;
	if (cnt >= 0)
		return NULL;
...
char	*make_fstring_var(const char *var_name)
{
	...
	upper(tmp_var);
	if ((var = find_ext_fset_var(tmp_var)))
		return malloc_strdup(var->string);
	...
	{
		case STR_TYPE_VAR:
			ret = malloc_strdup(fset_array[msv_index].string);
			break;
		case INT_TYPE_VAR:
			ret = malloc_strdup(ltoa(fset_array[msv_index].integer));
			break;
		case BOOL_TYPE_VAR:
			ret = malloc_strdup(var_settings[fset_array[msv_index].integer]);
			break;
		case CHAR_TYPE_VAR:
			ret = malloc_dupchar(fset_array[msv_index].integer);
			break;
	}
	return ret;
}
...
#ifdef GUI
BX_IrcVariable *return_fset_var(int nummer)
{
