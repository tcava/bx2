...
void flush_mode_all(Channel *chan)
{
char buffer[BIG_BUFFER_SIZE+1];
...
void add_mode(Channel *chan, const char *mode, int plus, char *nick, char *reason, int max_modes)
{
char buffer[BIG_BUFFER_SIZE+1];
/*
KICK $C nick :reason
...
BUILT_IN_COMMAND(dodeop)
{
	...
	if (!strcmp(command, "DEHOP"))
		...
		command = "o";
	else if (!strcmp(command, "DEVOICE") || !strcmp(command, "UNVOICE"))
		command = "v";
...
BUILT_IN_COMMAND(dokick)
{
	...
	if (!(chan = prepare_command(&server, (to && !strcmp(to, "*"))? NULL : to, NEED_OP)))
		return;
	l = message_from(chan->channel, LEVEL_KICK);
...
BUILT_IN_COMMAND(kickban)
{
	...
	if (!(chan = prepare_command(&server, to, NEED_OP)))
		return;
	set_display_target(chan->channel, LEVEL_KICK);
	if (command)
	{
...
{
	if (args && *args)
		set_default_bantype(*args, NULL);
// XXX: bx passes a "helparg" to each command. set_default_bantype doesn't use it, so pass NULL
